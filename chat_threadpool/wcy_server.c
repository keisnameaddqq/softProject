...
void listen(){
    ...
    Thread_Pool* pool=create_thread_pool(config->threadpool_size,config->log_threadpool);
    if(pool==NULL){
        add_log_server("EORROR: create_thread_pool pool failure");
        exit(1);
    }
    ...
    while (1){
        ...
        if (FD_ISSET(fifo_fd_resgister, &temp_set)){
            ...
            if(read(fifo_fd_resgister, &user, sizeof(user))>0){
                ...
                thread_pool_add(pool,handle_register,(void *)(&user));
            }
        }
        if (FD_ISSET(fifo_fd_login, &temp_set)){
            ...
            if(read(fifo_fd_login, &user, sizeof(user))>0){
                ...
                thread_pool_add(pool,handle_login,(void *)(&user));
            }
        }
        if (FD_ISSET(fifo_fd_logout, &temp_set)){
            ...
            if(read(fifo_fd_logout, &user, sizeof(user))>0){
                ...
                thread_pool_add(pool,handle_logout,(void *)(&user));
            }
        }
        if (FD_ISSET(fifo_fd_chat, &temp_set)){
            ...
            if(read(fifo_fd_chat, &meg, sizeof(meg))>0){
                ...
                thread_pool_add(pool,handle_sendmsg,(void *)(&meg));
            }   
        }
    }
